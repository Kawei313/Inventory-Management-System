...
class InventoryManagementSystem:
    ...
    def logout(self):
        answer = messagebox.askyesno("Confirm Logout", "Bạn có chắc chắn muốn đăng xuất không?")
        if answer:
            self.root.destroy()  # Đóng cửa sổ hiện tại
            import subprocess, sys, os
            python_exec = sys.executable
            login_path = os.path.join(os.path.dirname(__file__), "login_auto_role.py")
            subprocess.Popen([python_exec, login_path])
